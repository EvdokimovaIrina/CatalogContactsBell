...
public class ControllerHTMLImpl implements ControllerHTML {
    ...
    public String showContactListStr(Integer numberGroup) throws DaoException {
        ...
        String strHtml = menuListHTML(contactList, numberGroup);
        return strHtml;
    }

    private String getNamegroup(Integer numberGroup) throws DaoException {
        Group group = groupService.findByNumber(numberGroup);
        return group.getName();
    }

    private String menuListHTML(List<Contact> contactList, Integer numberGroup) throws DaoException {
        ...
        String strHtml = "<!DOCTYPE HTML>" +
                ...
                "</form><br>" +
                "<p><h3>Контакты " + (numberGroup == null ? "" : "группы: "+getNamegroup(numberGroup)) + "</h3></p></body></html>" +
    ...
    public String showGroupList() throws DaoException {
        ...
        for (Group group : groupList) {
            strHtml = strHtml + "<tr> " +
                    ...
                    "<td><input type=\"hidden\" name=\"idgroup\" value=\"" + group.getNumber() + "\"/>" +
                    "<input type=\"text\" name=\"namegroup\" value =" + group.getName() + " />" +
                    "</td>" +
                    "<td><input type=\"hidden\" name=\"buttonaction\" value=\"changegroupname\"/>" +
                    "<input type=\"submit\" value=\"Сохранить наименование\" />" +
                    "</td>" +
                    "</form>" +
                    "<form action=\"contacts\" method=\"POST\">" +
                    "<input type=\"hidden\" name=\"idgroup\" value=\"" + group.getNumber() + "\"/>" +
                    "<td><input type=\"hidden\" name=\"buttonaction\" value=\"showContactListGroup\"/>" +
                    "<input type=\"submit\" value=\"Показать список контактов\" /></td>" +
                    "</form>" +
                    "<form action=\"groups\" method=\"POST\">" +
                    "<input type=\"hidden\" name=\"idgroup\" value=\"" + group.getNumber() + "\"/>" +
                    "<td><input type=\"hidden\" name=\"buttonaction\" value=\"delete\"/>" +
                    "<input type=\"submit\" value=\"Удалить группу\" /></td>" +
                    "</form>" +
                    "</tr>";
        }
    ...
    public String findByName(String name) throws DaoException {
        ...
        String strHtml = menuListHTML(contactList, null);
        return strHtml;
    }
