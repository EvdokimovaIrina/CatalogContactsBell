...
CREATE OR REPLACE FUNCTION InsertUser(u_login VARCHAR,u_password VARCHAR) RETURNS VOID AS
$BODY$
BEGIN
    INSERT INTO "TUser" (user_id,
    						user_login,
                            user_password)
    	 VALUES (
         (SELECT max(user_id) FROM "TUser" )+1,
         u_login,
         u_password)
...
CREATE OR REPLACE FUNCTION ChangeUser(u_id INT,u_login VARCHAR,u_password VARCHAR) RETURNS VOID AS
$BODY$
BEGIN
...
CREATE OR REPLACE FUNCTION getUsers(u_id INT DEFAULT 0) RETURNS SETOF "TUser" AS
$BODY$
DECLARE
  r "TUser";
BEGIN
